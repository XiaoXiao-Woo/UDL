...
def get_max_memory(device):
    mem = torch.cuda.max_memory_allocated(device=device)
    mem_mb = torch.tensor([mem / (1024 * 1024)], dtype=torch.int)
...
def args_to_dict(args, keys, inplace=False):
    if inplace:
        return {k: args.pop(k) for k in keys if k in args}
    else:
        return {k: getattr(args, k) for k in keys if k in args}
...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        if not is_dist_avail_and_initialized():
            return
        t = torch.tensor(
            [self.val, self.count, self.total], dtype=torch.float64, device="cuda"
        )
        dist.barrier()
        dist.all_reduce(t)
        t = t.tolist()
    ...
    def __str__(self):
        ...
        #     value=self.value)
        return self.fmt.format(
            median=self.median, avg=self.avg, max=self.max, value=self.val, std=self.std
        )
...
class MetricLogger(object):
    ...
    def __init__(
        self, logger=None, delimiter="\t", dist_print=0, window_size=20, eval=False
    ):
        self.meters = defaultdict(
            partial(SmoothedValue, window_size=window_size, eval=eval)
        )
