...
class EMA(Module):
    ...
    def __init__(
        ...
        self.state_include_online_model = state_include_online_model and include_online_model
    ...
    def state_dict(self):
        if self.state_include_online_model:
            if is_model_parallel_initialized(self.online_model):
                ema_model_state_dict = extract_model_from_parallel(self.ema_model).state_dict()
                online_model_state_dict = extract_model_from_parallel(self.online_model).state_dict()
                return {'ema_model': ema_model_state_dict, 'online_model': online_model_state_dict}
            
            return {'ema_model': self.ema_model.state_dict(), 'online_model': self.online_model.state_dict()}
        else:
            if torch.distributed.is_initialized() or isinstance(self.online_model, torch.nn.parallel.DistributedDataParallel):
    ...
    def load_state_dict(self, state_dict):
        ...
        if self.state_include_online_model and state_dict.get('online_model', None) is not None:
            if is_model_parallel_initialized(self.online_model):
                self.ema_model.module.load_state_dict(state_dict['ema_model'])
                self.online_model.module.load_state_dict(state_dict['online_model'])
            else:
        ...
        # only has ema_model
            ...
            if is_model_parallel_initialized(self.online_model):
                self.ema_model.module.load_state_dict(state_dict)
            else:
                self.ema_model.load_state_dict(state_dict)
            print_log('No online_model is loaded, please make sure you have set `online_model`!')
