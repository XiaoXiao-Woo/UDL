...
def showimage8(images, unnormlize=2047.0, first_channel=False, channels=[0, 2, 4]):
    '''
    :param images: [H,W,C] or [C,H,W]
    :param unnormlize: wv3, wv2, qb: 2047.0; gf2: 1023.0
    :param first_channel: False
    :return: [H,W,C]
    '''
    ...
    if isinstance(images, torch.Tensor):
        ...
        output = images[..., channels] * torch.tensor(unnormlize)
        output = torch.clamp(output, 0, 2047)
        output = output.cpu().detach().numpy()
    else:
        unnormlize = np.where(max(float(images.max()), 1.0) > 1.0, 1.0, unnormlize)
        output = images[..., channels] * unnormlize
        output = np.clip(output, 0, 2047)
    try:
        norm_image = linstretch(output.copy())
