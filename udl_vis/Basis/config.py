...
def get_compose_yml(full_config_path):
...
class getAttrCLS:
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.all_keys = get_keys(cfg)

    def __call__(self, keys):
        return get_nested_attr(self.cfg, filter_keys(self.all_keys, keys))
...
class Config:
    ...
    def fromstring(cfg_str, file_format):
        ...
        with tempfile.NamedTemporaryFile(
            "w", suffix=file_format, delete=False
        ) as temp_file:
    ...
    def auto_argparser(description=None):
        ...
        partial_parser.add_argument(
            "--config", help="config file path", default="../../dev/config_detr.yml"
        )
    ...
    def __init__(self, cfg_dict=None, cfg_text=None, filename=None):
        ...
        elif not isinstance(cfg_dict, (dict, Namespace, DictConfig)):
            raise TypeError(
                "cfg_dict must be a dict or Namespace, but " f"got {type(cfg_dict)}"
            )
        for key in cfg_dict:
    ...
    def pretty_text(self):
        ...
        def _format_basic_types(k, v, use_mapping=False):
            if isinstance(v, str):
                ...
                v_str = str(v)
                if any(key in v_str for key in replace_dict.keys()):
                    v_str = pattern.sub(
                        lambda match: replace_dict[match.group(0)], v_str
                    ).replace(" ", "")
        ...
        text, _ = FormatCode(text.replace("\\", "/"), style_config=yapf_style)
