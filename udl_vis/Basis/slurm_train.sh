...
srun -p ${PARTITION} \
    ...
    ${SRUN_ARGS} \
    python -u dr_main.py --launcher="slurm" #${PY_ARGS}

#srun -p defq -J test -n 2 --nodelist=node[004-005] --ntasks-per-node=2 --export=cuda_home python -u test_slurm.py
#srun --partition=defq --job-name=rain -n 1 --nodelist=node004 --gres=gpu:8 --ntasks-per-node=8 python -u main.py --launcher="slurm
#sed -i "s/\r//" slurm_train.sh
# srun -p defq -J test -n 1 --nodelist=node[004] --ntasks-per-node=1 python -u main.py --launcher slurm
