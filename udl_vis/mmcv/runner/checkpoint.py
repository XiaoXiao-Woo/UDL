...
def load_state_dict(module, state_dict, prefix="", strict=False, logger=None):
    ...
    if unexpected_keys or missing_keys:
        print(f"source state_dict: {list(state_dict.keys())[0]}")
        if is_module_wrapper(module):
            module = module.module
        print(f"model: {list(module.state_dict().keys())[0]}")
...
def load_checkpoint(
    ...
    if not os.path.isfile(filename) and work_dir is not None:
        ...
        if resume_mode == "best":
            _, best_k_fname = get_best_k_model(
                os.path.join(work_dir, "checkpoint"), None
            )
...
def save_checkpoint(  # model,
    ...
    if "model" not in meta:
        ...
        for name, sub_meta in meta.items():
            ...
            checkpoint[name] = {
                "meta": sub_meta,
                "state_dict": model.state_dict()#weights_to_cpu(get_state_dict(model)),
            }
        ...
        checkpoint = {
            "meta": meta,
            "state_dict": model.state_dict(),#weights_to_cpu(model.model.state_dict()),
        }
        # save optimizer state dict in the checkpoint
        if isinstance(optimizer, Optimizer):
