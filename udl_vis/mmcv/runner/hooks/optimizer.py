...
from torch.nn.utils import clip_grad, clip_grad_value_

from udl_vis.mmcv.utils import TORCH_VERSION, _BatchNorm, digit_version
from ..dist_utils import allreduce_grads
from ..fp16_utils import LossScaler, wrap_fp16_model
from .hook import HOOKS, Hook
from udl_vis.Basis.optim.optimizer import get_grad_norm
try:
...
class OptimizerHook(Hook):
    ...
    def __init__(self, grad_clip_norm=None, grad_clip_value=None, detect_anomalous_params=False):
        self.grad_clip_norm = grad_clip_norm
        self.grad_clip_value = grad_clip_value
        self.detect_anomalous_params = detect_anomalous_params

    def clip_norm_(self, params, accelerator=None):
        # params = list(
        #     filter(lambda p: p.requires_grad and p.grad is not None, params))
        params, grad_norm = get_grad_norm(params)
        if len(params) > 0 and self.grad_clip_norm:
            # **self.grad_clip
            if accelerator is None:
                return params, clip_grad.clip_grad_norm_(params, self.grad_clip_norm)
            else:
                return params, accelerator.clip_grad_norm_(params, self.grad_clip_norm)
        else:
            return params, grad_norm

    def clip_value_(self, params, accelerator=None):
        # params = list(
        #     filter(lambda p: p.requires_grad and p.grad is not None, params))
        if len(params) > 0 and self.grad_clip_value:
            # **self.grad_clip
            if accelerator is None:
                clip_grad.clip_grad_value_(params, self.grad_clip_value)
            else:
                accelerator.clip_grad_value_(params, self.grad_clip_value)

    def clip_grad(self, runner):
        if not hasattr(runner.model, 'train'):
            params = runner.model.model.parameters()
            params, grad_norm = self.clip_norm_(params)
            self.clip_value_(params)
        else:
        ...
        if isinstance(runner.optimizer, dict):
            ...
            for name, optim in runner.optimizer.items():
                ...
                if hasattr(runner, 'accelerator'):
                    runner.accelerator.backward(runner.outputs[f'{name}_loss'])
                else:
            ...
            if hasattr(runner, 'accelerator'):
                runner.accelerator.backward(runner.outputs['loss'])
            else:
                runner.outputs['loss'].backward()
            self.clip_grad(runner)
    ...
    def detect_anomalous_parameters(self, loss, runner):
        ...
        traverse(loss.grad_fn)
        for n, p in runner.model.model.named_parameters():
            if p not in parameters_in_graph and p.requires_grad:
                logger.log(
                    level=logging.ERROR,
                    msg=f'{n} with shape {p.size()} is not '
...
class AcceleratorOptimizerHook(OptimizerHook):
    """A hook contains custom operations for the optimizer.
    ...
    def after_train_iter(self, runner):
        if isinstance(runner.optimizer, dict):
            if isinstance(runner.model, dict):
                for name, m in runner.model.items():
                    grad_norm = self.clip_norm_(m.parameters())
                    # if grad_norm is not None:
                    # Add grad norm to the logger
                    # runner.outputs['num_samples'] 用于计算平均的，MetricLogger改进了log_buffer的几个计算，这里不需要了
                    runner.log_buffer.update_dict({f'{name}_grad_norm': float(grad_norm)})
    ...
    class Fp16OptimizerHook(OptimizerHook):
        ...
        def after_train_iter(self, runner):
            ...
            if not has_overflow:
                ...
                grad_norm = self.clip_norm_(fp32_weights)
